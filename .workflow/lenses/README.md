...
Use `save` for session outputs: `.session/inbox/**` for staged inputs, `.session/drafts/**` for unapproved working artifacts, and `.session/accepted/**` for accepted session-level sources. Formal project knowledge goes to `docs/**` through `sync`. Code-adjacent reading notes go to `src/**/README.md`.
