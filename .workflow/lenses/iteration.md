...
- Use `persist` for work-in-progress judgments in `.session/drafts/**`.
- Use `persist` for accepted session judgments in `.session/accepted/**` when accepted intent is explicit.
