...
### 3. 新增/修改文件: `shared_core/indicators/atr.py`

- **变更类型**: 新增
- **变更描述**:
  - 提供 `compute_atr`（True Range/ATR）基础指标计算函数

### 4. 新增/修改文件: `shared_core/indicators/realized_vol.py`

- **变更类型**: 新增
- **变更描述**:
  - 提供 `compute_realized_vol`（log return std）基础指标计算函数

### 5. 新增/修改文件: `shared_core/indicators/momentum.py`

- **变更类型**: 新增
- **变更描述**:
  - 提供 `compute_momentum` 与 `compute_momentum_vol` 基础指标计算函数

### 6. 新增/修改文件: `shared_core/indicators/vol_mom_dca_metrics.py`

- **变更类型**: 新增
- **变更描述**:
  - 组合 ATR/realized vol/momentum 指标，计算混合波动率与动量得分
  - 仅保留 DCA 组合逻辑，避免重复实现基础指标

### 7. 新增/修改文件: `shared_core/strategies/vol_mom_dca.py`
...
### 8. 新增/修改文件: `shared_core/inventory/rebalance.py`
...
### 9. 新增/修改文件: `backtest_app/app_utils/yaml.py`
...
### 10. 新增/修改文件: `backtest_app/data_providers/base.py`

- **变更类型**: 修改
- **变更描述**:
  - `fetch()` 支持按 `MarketDataRequest.fields` 返回 OHLC 列

### 11. 新增/修改文件: `backtest_app/data_providers/adapters/simulator.py`

- **变更类型**: 修改
- **变更描述**:
  - 支持输出 OHLC 字段（可用简单合成逻辑）

### 12. 新增/修改文件: `backtest_app/app/settings/loader.py`

- **变更类型**: 修改
- **变更描述**:
  - 读取分层配置结果，增加 vol_mom_dca 配置字段并绑定到 `VolMomDcaParams`

### 13. 新增/修改文件: `backtest_app/app/services/runner.py`
...
### 14. 新增/修改文件: `backtest_app/engines/backtest/engine.py`

- **变更类型**: 修改
- **变更描述**:
  - 增加“目标权重序列”执行路径（如 `Portfolio.from_orders` / `from_weights`）

### 15. 新增/修改文件: `configs/base.yaml`
...
### 16. 新增/修改文件: `configs/strategies/vol_mom_dca.yaml`

- **变更类型**: 新增
- **变更描述**:
  - vol_mom_dca 策略参数（与 `VolMomDcaParams` 对齐）

### 17. 新增/修改文件: `tests/test_backtest_app/test_vol_mom_dca_strategy.py`
...
### 18. 新增/修改文件: `tests/test_backtest_app/test_config_loader_includes.py`
...
2. [ ] 在 `shared_core/indicators` 新增基础指标模块（ATR、realized vol、momentum），并补充单元测试。
3. [ ] 新增 `shared_core/indicators/vol_mom_dca_metrics.py` 作为组合指标层。
