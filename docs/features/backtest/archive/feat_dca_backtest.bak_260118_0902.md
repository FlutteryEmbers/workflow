...
> **目标**: 将 `.sandbox/dca_trade/dca.py` 的 DCA 权重分配逻辑接入 `backtest_app`，通过库存/仓位管理执行再平衡，并支持在回测中按配置选择 DCA 策略输出结果。
> **范围**:
>
> - [x] 核心: DCA 权重计算 + 仓位再平衡 + 回测执行链路集成
...
- **方案 B (Robust)**: 支持 OHLC + 定期再平衡权重序列，回测引擎走“目标权重 + 再平衡 allocator”路径。 - [ ] ✅ 采纳 (理由: 与 DCA 逻辑一致，架构职责清晰)
...
- **API**: `MarketDataRequest` 结构扩展，新增 DCA Strategy 与 RebalanceAllocator 接口
...
```mermaid
flowchart LR
    new_cli["CLI"] --> new_runner["Runner"] --> new_strategy["DcaStrategy"] --> new_allocator["RebalanceAllocator"] --> new_engine["BacktestEngine"] --> new_report["ReportBuilder"]
...
### 12. 新增/修改文件: `tests/test_backtest_app/test_dca_strategy.py`
...
6. [ ] 扩展 `backtest_app/app/settings/loader.py` 与 `configs/config.yaml` 以解析 DCA 配置。
7. [ ] 修改 `backtest_app/app/services/runner.py` 增加 DCA 分支、allocator 调用与输出。
