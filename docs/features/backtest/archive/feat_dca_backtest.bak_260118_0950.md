...
### 2. 新增/修改文件: `shared_core/models/strategies.py`

- **变更类型**: 新增
- **变更描述**:
  - 新增 `VolMomDcaParams` 策略配置模型

### 3. 新增/修改文件: `shared_core/indicators/basic.py`

- **变更类型**: 新增
- **变更描述**:
  - 统一提供基础指标计算函数：`compute_atr`、`compute_realized_vol`、`compute_momentum`、`compute_momentum_vol`

### 4. 新增/修改文件: `shared_core/indicators/strategies.py`
...
### 5. 新增/修改文件: `shared_core/strategies/vol_mom_dca.py`
...
### 6. 新增/修改文件: `shared_core/inventory/rebalance.py`
...
### 7. 新增/修改文件: `backtest_app/app_utils/yaml.py`
...
### 8. 新增/修改文件: `backtest_app/data_providers/base.py`

- **变更类型**: 修改
- **变更描述**:
  - `fetch()` 支持按 `MarketDataRequest.fields` 返回 OHLC 列

### 9. 新增/修改文件: `backtest_app/data_providers/adapters/simulator.py`

- **变更类型**: 修改
- **变更描述**:
  - 支持输出 OHLC 字段（可用简单合成逻辑）

### 10. 新增/修改文件: `backtest_app/app/settings/loader.py`

- **变更类型**: 修改
- **变更描述**:
  - 读取分层配置结果，增加 vol_mom_dca 配置字段并绑定到 `VolMomDcaParams`

### 11. 新增/修改文件: `backtest_app/app/services/runner.py`
...
### 12. 新增/修改文件: `backtest_app/engines/backtest/engine.py`

- **变更类型**: 修改
- **变更描述**:
  - 增加“目标权重序列”执行路径（如 `Portfolio.from_orders` / `from_weights`）

### 13. 新增/修改文件: `configs/base.yaml`
...
### 14. 新增/修改文件: `configs/strategies/vol_mom_dca.yaml`

- **变更类型**: 新增
- **变更描述**:
  - vol_mom_dca 策略参数（与 `VolMomDcaParams` 对齐）

### 15. 新增/修改文件: `tests/test_backtest_app/test_vol_mom_dca_strategy.py`
...
### 16. 新增/修改文件: `tests/test_backtest_app/test_config_loader_includes.py`
...
1. [ ] 在 `shared_core/models` 新增 `market_data.py` 与 `strategies.py`，并更新 `shared_core/schemas` 为兼容 re-export。
