...
> **目标**: 将 `.sandbox/dca_trade/dca.py` 的 DCA 权重分配逻辑以 `vol_mom_dca` 策略名接入 `backtest_app`，通过库存/仓位管理执行再平衡，并支持在回测中按配置选择该策略输出结果。
> **范围**:
>
...
- [ ] **Config File**: 在 `configs/config.yaml` 增加 `strategies.vol_mom_dca` 配置块与 `rebalance_frequency`。
- [ ] **Env Vars**: 无。
...
- [ ] AC1: 在 `configs/config.yaml` 设置 `strategies.active: vol_mom_dca` 时，可完成回测并在 `outputs/` 输出结果文件。
...
- [ ] **配置检查**: 是，需扩展 `configs/config.yaml`，理由：vol_mom_dca 参数与再平衡频率。
...
- **模块**: `shared_core/models`, `shared_core/strategies`, `shared_core/indicators`, `shared_core/inventory`, `backtest_app/engines`, `backtest_app/app/settings`, `backtest_app/app/services`
...
- allocator 输出权重与引擎执行方式不一致导致回测误差。
...
```mermaid
flowchart LR
    new_cli["CLI"] --> new_runner["Runner"] --> new_strategy["VolMomDcaStrategy"] --> new_allocator["RebalanceAllocator"] --> new_engine["BacktestEngine"] --> new_report["ReportBuilder"]
...
### 6. 新增/修改文件: `backtest_app/data_providers/base.py`

- **变更类型**: 修改
- **变更描述**:
  - `fetch()` 支持按 `MarketDataRequest.fields` 返回 OHLC 列

### 7. 新增/修改文件: `backtest_app/data_providers/adapters/simulator.py`

- **变更类型**: 修改
- **变更描述**:
  - 支持输出 OHLC 字段（可用简单合成逻辑）

### 8. 新增/修改文件: `backtest_app/app/settings/loader.py`

- **变更类型**: 修改
- **变更描述**:
  - 增加 vol_mom_dca 配置字段并绑定到 `VolMomDcaParams`

### 9. 新增/修改文件: `backtest_app/app/services/runner.py`
...
### 10. 新增/修改文件: `backtest_app/engines/backtest/engine.py`

- **变更类型**: 修改
- **变更描述**:
  - 增加“目标权重序列”执行路径（如 `Portfolio.from_orders` / `from_weights`）

### 11. 新增/修改文件: `configs/config.yaml`

- **变更类型**: 修改
- **变更描述**:
  - 新增 `strategies.vol_mom_dca` 配置块（与 `VolMomDcaParams` 对齐）

### 12. 新增/修改文件: `tests/test_backtest_app/test_vol_mom_dca_strategy.py`
...
5. [ ] 扩展 `MarketDataProvider` 与 `SimulatorProvider` 支持 OHLC 字段。
6. [ ] 扩展 `backtest_app/app/settings/loader.py` 与 `configs/config.yaml` 以解析 vol_mom_dca 配置。
7. [ ] 修改 `backtest_app/app/services/runner.py` 增加 vol_mom_dca 分支、allocator 调用与输出。
8. [ ] 修改 `BacktestEngine` 以支持目标权重序列回测路径。
9. [ ] 增加测试用例并补充文档/示例配置。
...
- **手动验证**:
  - 运行 `python run.py --config configs/config.yaml --mode backtest --profile profile_A`，确认输出包含 vol_mom_dca 权重/再平衡信息。
