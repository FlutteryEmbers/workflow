...
- **Class/Module**: `shared_core/models/strategies.py`
- **Method Signature**:
...
- **Reason**: 策略相关模型统一收敛到 `shared_core/models/strategies.py`，满足架构规则与复用需求。

- **Class/Module**: `shared_core/models/frequency.py`
- **Method Signature**:
  ...
  FREQUENCY_ALIASES: dict[str, MarketFrequency] = {
      "D": MarketFrequency.ONE_DAY,
      "1D": MarketFrequency.ONE_DAY,
      "day": MarketFrequency.ONE_DAY,
  }

  def normalize_frequency(value: str) -> MarketFrequency:
      ...
  ```

- **Reason**: 全局频率映射与归一化入口，避免 provider 使用不一致字段。
...
- **Reason**: vol_mom_dca 需要 OHLC 字段，且频率必须使用标准化值。
...
- 频率规范与别名映射统一由 `shared_core/models/frequency.py` 维护。
...
- [ ] AC6: `MarketDataRequest` 能归一化频率别名，provider 仅接收标准频率值。
...
- **变更描述**:
  ...
  - 使用 `MarketFrequency` 并在模型内做频率归一化
  - `shared_core/schemas/market_data.py` 改为 re-export，保持旧引用兼容

### 2. 新增/修改文件: `shared_core/models/frequency.py`

- **变更类型**: 新增
- **变更描述**:
  - 定义 `MarketFrequency` 与 `normalize_frequency`
  - 维护频率别名映射表

### 3. 新增/修改文件: `shared_core/models/strategies.py`

- **变更类型**: 新增
- **变更描述**:
  - 新增 `VolMomDcaParams` 策略配置模型

### 4. 新增/修改文件: `shared_core/indicators/basic.py`

- **变更类型**: 新增
- **变更描述**:
  - 统一提供基础指标计算函数：`compute_atr`、`compute_realized_vol`、`compute_momentum`、`compute_momentum_vol`

### 5. 新增/修改文件: `shared_core/indicators/strategies.py`

- **变更类型**: 新增
- **变更描述**:
  - 集中策略组合指标实现（包含 `vol_mom_dca_metrics`）
  - 组合基础指标，计算混合波动率与动量得分

### 6. 新增/修改文件: `shared_core/strategies/vol_mom_dca.py`
...
### 7. 新增/修改文件: `shared_core/inventory/rebalance.py`
...
### 8. 新增/修改文件: `backtest_app/app_utils/yaml.py`
...
### 9. 新增/修改文件: `backtest_app/data_providers/base.py`

- **变更类型**: 修改
- **变更描述**:
  - `fetch()` 支持按 `MarketDataRequest.fields` 返回 OHLC 列

### 10. 新增/修改文件: `backtest_app/data_providers/adapters/simulator.py`

- **变更类型**: 修改
- **变更描述**:
  - 支持输出 OHLC 字段（可用简单合成逻辑）

### 11. 新增/修改文件: `backtest_app/app/settings/loader.py`

- **变更类型**: 修改
- **变更描述**:
  - 读取分层配置结果，增加 vol_mom_dca 配置字段并绑定到 `VolMomDcaParams`

### 12. 新增/修改文件: `backtest_app/app/services/runner.py`
...
### 13. 新增/修改文件: `backtest_app/engines/backtest/engine.py`

- **变更类型**: 修改
- **变更描述**:
  - 增加“目标权重序列”执行路径（如 `Portfolio.from_orders` / `from_weights`）

### 14. 新增/修改文件: `configs/base.yaml`
...
### 15. 新增/修改文件: `configs/strategies/vol_mom_dca.yaml`

- **变更类型**: 新增
- **变更描述**:
  - vol_mom_dca 策略参数（与 `VolMomDcaParams` 对齐）

### 16. 新增/修改文件: `tests/test_backtest_app/test_vol_mom_dca_strategy.py`
...
### 17. 新增/修改文件: `tests/test_backtest_app/test_config_loader_includes.py`

- **变更类型**: 新增
- **变更描述**:
  - 分层配置合并与覆盖顺序测试

### 18. 新增/修改文件: `tests/test_backtest_app/test_frequency_normalization.py`
...
1. [ ] 在 `shared_core/models` 新增 `market_data.py`、`frequency.py` 与 `strategies.py`，并更新 `shared_core/schemas` 为兼容 re-export。
2. [ ] 在 `shared_core/indicators/basic.py` 集中基础指标（ATR、realized vol、momentum），并补充单元测试。
3. [ ] 新增 `shared_core/indicators/strategies.py`，集中策略组合指标（含 vol_mom_dca）。
4. [ ] 新增 `shared_core/strategies/vol_mom_dca.py`，迁移算法与权重计算逻辑并复用指标模块。
5. [ ] 新增 `shared_core/inventory/rebalance.py`，实现再平衡 allocator。
6. [ ] 扩展 `backtest_app/app_utils/yaml.py` 以支持 `includes` 深度合并。
7. [ ] 扩展 `MarketDataProvider` 与 `SimulatorProvider` 支持 OHLC 字段。
8. [ ] 更新 `backtest_app/app/settings/loader.py` 适配分层配置。
9. [ ] 新增 `configs/base.yaml` 与 `configs/strategies/vol_mom_dca.yaml`，并调整示例运行说明。
10. [ ] 修改 `backtest_app/app/services/runner.py` 增加 vol_mom_dca 分支、allocator 调用与输出。
11. [ ] 修改 `BacktestEngine` 以支持目标权重序列回测路径。
12. [ ] 增加测试用例并补充文档/示例配置。
...
- **自动化测试**:
  ...
  - 基础指标：ATR/realized vol/momentum 的数值一致性与边界检查。
